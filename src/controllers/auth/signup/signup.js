...
export async function addmobilesignupoauth(rq, rs, pass) {
  let { token, mobile } = await rq.body;
  mobile = `+1${mobile}`;
  try {
    let testmobile =
      mobile &&
    ...
    if (Validator.isJWT(token || "") && testmobile) {
      const { id: payloadid, referal, type } = jwt.verify(
        token,
        _accesstokensecrete
      );
...
// setup merchant business

export async function merchantBussinessSetup(rq, rs, pass) {
  const { category, services } = await rq.body;
  const { userid } = await rq.params;

  try {
    await setmerchantbusiness(userid, category, services);
    let message = `Your business setup has done`;
